...
def countAuthors():
    ...
    db = connect()
    cursor = db.cursor()
    query = ("SELECT authors.name, count(authors.name) FROM log JOIN articles\n"
    " ON log.path ILIKE '%' || articles.slug JOIN authors ON articles.author\n"
    " = authors.id GROUP BY authors.name ORDER BY count DESC;")
    cursor.execute(query)
    results = cursor.fetchall()
    for result in results:
        print("%s - %s views") % (result[0], result[1])
...
def countErrors():
    ...
    db = connect()
    cursor = db.cursor()
    query = ("SELECT errors.day, errors.count AS errors, sum_all_events.total\n"
    " AS total, ((errors.count/sum_all_events.total) * 100) AS percent FROM\n"
    " errors JOIN sum_all_events ON errors.day = sum_all_events.day WHERE\n"
    " (errors.count/sum_all_events.total) * 100 > 1;")
    cursor.execute(query)
    results = cursor.fetchall()
    for result in results:
